 ...
 * 메인 스크래핑 시스템
 */
class MainScrapingSystem {
  constructor() {
    this.apiClient = new ApiClient(apiConfig.influencerApi);
    this.databaseService = new DatabaseService(dbConfig.mysql);
    this.influencerScraper = new InfluencerScraper();
  }
  ...
  async runScrapingProcess() {
    try {
      ...
      this.influencerScraper.addInfluencersToQueue(influencerIds);

      // 3. 순차적으로 인플루언서 스크래핑 및 데이터 저장
      const results = await this.influencerScraper.processAllInfluencers();
  ...
  async getSystemStatus() {
    try {
      ...
      const queueStatus = this.influencerScraper.getQueueStatus();
...
module.exports = MainScrapingSystem; 