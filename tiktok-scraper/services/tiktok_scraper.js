  ...
  async processNextInfluencer() {
    ...
    try {
      ...
      if (!this.databaseService) {
        ...
        console.log('⚠️ 데이터베이스 서비스가 없어 기존 방식으로 처리');
      ...
      if (config.scraping.enableFollowerScraping && this.databaseService) {
        ...
        console.log(`⏭️ 팔로워 수집 건너뛰기 (설정에 의해 비활성화)`);
      }
      ...
      if (config.scraping.enablePostDetailScraping && this.databaseService) {
        ...
        console.log(`⏭️ 게시물 상세 정보 수집 건너뛰기 (설정에 의해 비활성화)`);
      }
      ...
      if (config.scraping.enableCommentScraping && this.databaseService) {
        ...
        console.log(`⏭️ 댓글 수집 건너뛰기 (설정에 의해 비활성화)`);
      }
  ...
  async convertSinglePostToDbFormat(post, username) {
    try {
      ...
      const postRecord = {
        ...
        upload_date: new Date(), // 정확한 날짜는 모르므로 현재 시간
        length: 0,
        cover: '',
        video_url: '',
        music_title: '',
    ...
    try {
      ...
      if (postData) {
        ...
        return {
          ...
          upload_date: postData.upload_date || new Date().toISOString(),
      ...
      try {
        ...
        if (detailedPostData) {
          ...
          if (foundPost) {
            ...
            console.log(`  ⚠️ 게시물을 찾을 수 없음: ${postUrl}`);
          }
        }
  ...
  async scrapeAndSaveComments(postUrls, profileId) {
    ...
    for (let i = 0; i < postUrls.length; i++) {
      const postUrl = postUrls[i];
        ...
        console.log(`⏱️ 소요 시간: ${postDurationMinutes}분 (${Math.round(postDuration / 1000)}초)`);
        ...
        if (streamResult && streamResult.success) {
          ...
          console.log(`  ⚠️ 스트림 수집 실패: ${postUrl}`);
